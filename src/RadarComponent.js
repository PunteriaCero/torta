...
function RadarComponent({ data, onClick, config }) {
  ...
  useEffect(() => {
    ...
    if (sectionsData) {
      ...
      sections
        ...
        .attr("fill-opacity", sectionOpacity)
        .style("cursor", "pointer")
        .style("filter", (d) =>
          d.data.selected
            ? "drop-shadow(0px 1.5px 1.5px rgba(0, 0, 0, 0.6))"
            : "drop-shadow(0px 1.5px 1.5px rgba(0, 0, 0, 0.5))"
        )
      ...
      sections
        ...
        .attr("fill", (d) => (d.data.selected ? "white" : d.data.color)) // Color del fondo -- En caso de estar seleccionado debe ser verde
        .attr("rx", 4) // Radio horizontal de las esquinas
        .style("cursor", "pointer")
        .attr("stroke", "black") // En caso de estar seleccionado debe ser negro
      ...
      sections
        ...
        .style("fill", (d, i) =>
          d.data.selected ? "rgb(100,100,100)" : "whitesmoke"
        )
        .style("filter", (d) =>
          d.data.selected ? "" : "drop-shadow(0px 0px 0.7px rgba(0, 0, 0, 1))"
        );
    ...
    if (targetsData) {
      ...
      point
        ...
        .style("filter", (d) =>
          d.data.selected
            ? `drop-shadow(0px 0px 3px ${d.data.color})`
            : "drop-shadow(0px 1.5px 1.5px rgba(0, 0, 0, 0.5))"
        )
  ...
  }, [sectionsData, targetsData, baseCircles, colorLines, height, numLines, onClick, radius, sectionOpacity, width]);
