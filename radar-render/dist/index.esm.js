...
var setPositionCircle = function setPositionCircle(newSection, reference, start) {
  var circleStart = d3.select(reference.classSelectStart);
  var circleEnd = d3.select(reference.classSelectEnd);
  if (start) {
...
var generateReferencesDOM = function generateReferencesDOM(section) {
  ...
  referencesClass['existCircleStart'] = d3.select(referencesClass.classSelectStart).node();
  referencesClass['existCircleEnd'] = d3.select(referencesClass.classSelectEnd).node();
...
var useRadarComponent = function useRadarComponent(_ref) {
  ...
  var resetCircles = function resetCircles(data) {
    data.forEach(function (item) {
      if (!item.selected) {
        var circleElements = d3.selectAll(".item-".concat(item.label)).nodes();
...
function RadarComponent(_ref) {
  ...
  var radarConfigRef = useRef(initialConfig);
  ...
  useEffect(function () {
    ...
    var svg = d3.select('g.radar-component');
    if (svg.empty()) {
      // If it doesn't exist, create a new one
      svg = d3.select(svgRef.current).attr('width', width).attr('height', height).append('g').attr('class', 'radar-component')
    ...
    var lineAngles = d3.range(numLines).map(function (i) {
    ...
    if (sections && showSections) {
      ...
      var path = d3.arc().outerRadius(function (d) {
    ...
    if (targets && !showSections) {
      ...
      var pathPoint = d3.arc().outerRadius(function (d) {
  ...
  useEffect(function () {
    var timeout = setTimeout(function () {
      ...
      if (!isExecuted) {
        ...
        var circleElement = d3.select("#section-".concat(existSelected === null || existSelected === void 0 ? void 0 : existSelected.label)).node();
        if (circleElement) {
  ...
  }, north), /*#__PURE__*/React.createElement("svg", {
    width: "800px",
    ref: svgRef
  })));
}
